...
class Search:
    ...
    def absearch(self, ply: int, alpha: int, beta: int) -> list:
        if self.checkTime():
            return []
        self.pv = self.eval.evaluate(ply, alpha, beta, self.board)
        # pprint.pprint(self.memory)
        return self.memory

    def iterativeDeepening(self, ply) -> None:
        self.zmb_start = [0, [28], [21, 25], [23, 24], [20, 26], 24, 23]
        # self.zmb_chess = [0, 28, 27, 26, 25, 24, 23]
        # self.zmb_start = [0, 1, 2, 3, 4, 5, 6]
        self.zmb_table = [
        ...
        for t in range(MAX_TEMP):
            ...
            for p in range(1, ply):
                alpha = -score
                beta = score
                memory = self.absearch(p, alpha, beta)
